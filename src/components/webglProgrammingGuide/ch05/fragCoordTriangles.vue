...
<script>
import vShaderSource from "@/shaders/vfragCoordTriangles.glsl";
import fShaderSource from "@/shaders/ffragCoordTriangles.glsl";
...
export default {
  ...
  methods: {
    ...
    loadPoint() {
      ...
      let u_width = gl.getUniformLocation(gl.program, "u_width"); //此方法无任何属性和方法
      if (u_width < 0) {
        alert("获取u_width位置属性失败");
      }
      gl.uniform1f(u_width, gl.drawingBufferWidth);
      let u_height = gl.getUniformLocation(gl.program, "u_height"); //此方法无任何属性和方法
      if (u_height < 0) {
        alert("获取u_height位置属性失败");
      }
